...
if [[ " ${BuildLayers[@]} " =~ " os " ]]; then . $In4_Exec_Path/_base/build/2.scenario/2.kern-packages.sh; fi
if [[ $DeployOsMode == "vm_xen" ]]; then . $In4_Exec_Path/vm_xen/build/2.scenario/2.vm_xen-packages.sh; fi
if [[ " ${BuildLayers[@]} " =~ " unit " ]]; then . $In4_Exec_Path/hw_chroot/build/2.scenario/2.hw_chroot-packages.sh; fi
if [[ " ${BuildLayers[@]} " =~ " os " ]]; then . $In4_Exec_Path/_base/build/2.scenario/3.kern-step2_boot.sh; fi
...
if [[ " ${BuildLayers[@]} " =~ " unit " ]]; then . $In4_Exec_Path/hw_chroot/build/2.scenario/9.hw_chroot-post.sh; fi
