...
if [[ $DeployOsMode == "vm_xen" ]] ; then 
    if losetup -j $BuildEnv/sysdata.raw; then sudo losetup -d /dev/$VmDiskLoopSysdata; fi
    if losetup -j $BuildEnv/$In4NamingOsSrvType.raw; then sudo losetup -d /dev/$VmDiskLoopSystem; fi
fi
