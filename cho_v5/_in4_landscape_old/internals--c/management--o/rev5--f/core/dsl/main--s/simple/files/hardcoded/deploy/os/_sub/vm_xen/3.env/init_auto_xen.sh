...
if [[ -f /etc/systemd/system/init_auto_xen.service ]]; then  
        ...
        cp /media/sysdata/in4/cho/cho_v5/in4_landscape/internals--c/linux_sys--o/boot--f/dracut/dsl/main--s/simple/files/hardcoded/51-block-xenvm_blacklist.conf /etc/modprobe.d/ ## BUG
        ...
        ###  DISABLE NET DHCP ### 
        cp /media/sysdata/in4/cho/in4_core/deploy/os/3.env/wtf/wickedd-dhcp /usr/lib/systemd/system/wickedd-auto4.service
        cp /media/sysdata/in4/cho/in4_core/deploy/os/3.env/wtf/wickedd-dhcp /usr/lib/systemd/system/wickedd-dhcp4.service
	...
	if [[ $View == "os" ]]; then 
		...
		echo "127.0.0.3 $NAME $SrvName" >> /etc/hosts
                ...
                sh /media/sysdata/in4/cho/in4_core/deploy/os/2.scenario/99.post_once.sh
