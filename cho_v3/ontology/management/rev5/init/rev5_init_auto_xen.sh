...
if [[ ! -f /media/sysdata/rev5/firstinit.hostip && ! -d /etc/faster/cmdb ]]; then  
	DOMID=`xenstore-read domid`
	NAME=`xenstore-read /local/domain/$DOMID/name`
	. /media/sysdata/rev5/static/naming.sh os $NAME
	
	if [[ $View == "os" ]]; then 
		#hostname
		cp /etc/hosts /etc/hosts.back
		cp /media/sysdata/rev5/static/hosts /etc/hosts
	...
	if [[ `echo $?` == 0 ]]; then 
		...
		#net
		cp /media/sysdata/rev5/static/ifcfg-tmpl /etc/sysconfig/network/ifcfg-eth0
