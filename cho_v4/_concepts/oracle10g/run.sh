...
if test -f /media/storage/as/oracle/_mv_svn/techpool/oracle.vars ; then
    . /media/storage/as/oracle/_mv_svn/techpool/oracle.vars 
fi
...
ora_environment()
{
 ...
 if [ ! -f /etc/oratab -a ! -d "$ORACLE_HOME" ]; then
  ...
  if [ "$1" = "start" ]; then
    echo -n " ${extd}SETTINGS $1 from /media/storage/as/oracle/_mv_svn/techpool/oracle.vars $norm"
    if [ ! -f /media/storage/as/oracle/_mv_svn/techpool/oracle.vars  ]; then
...
case "$1" in
  start)
    ...
    if [ "${SET_ORACLE_KERNEL_PARAMETERS:-no}" == "yes" ]; then
      ...
      if test -f /media/storage/as/oracle/_mv_svn/techpool/oracle.vars ; then
        echo "/media/storage/as/oracle/_mv_svn/techpool/oracle.vars  for explanations."
      else
